...
public class EmployeeController {
	...
	public List<Employee> getAllCacheEmployees() {
		...
		return empRepository.findAll().stream().filter(x -> x.getStatus().equals("Active"))
	...
	public long getEmployeeId(@Valid @RequestBody Employee emp) {
		...
		if (idEmployee < 0) {
			Employee empDB = empRepository.findByFirstNameAndMiddleInitialAndLastName(emp.getFirstName(),
					emp.getMiddleInitial(), emp.getLastName());
			if (empDB.getId() <= 0) {
				throw new EmployeeNotFoundException("employee with name: " + emp.getFirstName() + " "
						+ emp.getMiddleInitial() + " " + emp.getLastName() + " not found");
			}
			init();
			return empDB.getId();
		} else {
	...
	// update one employee
	@PostMapping(path = "/private/updateEmployeeById")
	public ResponseEntity<Object> updateEmployee(@Valid @RequestBody Employee emp)
	...
	private void updateOnlyNotNullFields(Class<Employee> empClass, Employee emp, Employee savedEmp)
	...
	// get employee model with full name
	@PostMapping(path = "/public/getEmployeeModelWithFullName")
	public Employee getEmployeeModel(@Valid @RequestBody Employee emp) {
		Employee empFound = empRepository.findByFirstNameAndMiddleInitialAndLastName(emp.getFirstName(),
				emp.getMiddleInitial(), emp.getLastName());
	...
	// delete one employee
	@DeleteMapping(path = "/private/deleteEmployee/{id}")
